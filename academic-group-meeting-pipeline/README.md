...
这是一套专为**学术文献组会汇报**设计的 **WorkBuddy AI Skill 流水线**。灵感来自「全息学术」公众号的方法论，经过工程化改造，将论文从 PDF 到完整汇报材料的全流程自动化。
...
| B2 | **ppt-architect** | `skills/ppt-architect/` | 🎨 PPT 视觉架构师 |
...
| C1 | **ppt-implement-custom** | `skills/ppt-implement-custom/` | 🎬 .pptx 文件生成器 |
| **主控** | **group-meeting-pipeline** | `skills/group-meeting-pipeline/` | 🚀 一键总控流水线 |
...
每个子文件夹包含一个 `SKILL.md`，即该 Skill 的完整指令定义。
...
3. **双PPT格式** — .pptx 用于分发编辑，网页PPT用于演示展示
4. **渐进式信息密度** — 从骨架→蓝图→逐字稿→Q&A，层层细化
...
└── skills/                      ← 所有 AI Skill 定义
    │
    ├── group-meeting-pipeline/  ← 🚀 主控：一键总控流水线 v2.1
    │   └── SKILL.md
    │
    ...
    │   └── SKILL.md
    │
    └── ppt-implement-custom/      ← C1: .pptx文件生成器
        └── SKILL.md               (含完整Python脚本模板)
```
