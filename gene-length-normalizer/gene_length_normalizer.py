...
def main():
    print("=" * 60)
    print("  基因长度偏差校正（RPKM/TPM/GC）")
    print("=" * 60)
    print()

    # === Input Parameters ===
    input_file = get_input("Input file path", "input.txt")
    output_file = get_input("Output file path", "output_gene_length_normalizer.txt")
    ...
    if not os.path.exists(input_file):
        ...
        with open(input_file, "w") as f:
            f.write("# Demo input file for gene_length_normalizer\n")
    ...
    try:
        with open(output_file, "w") as f:
            f.write("# gene_length_normalizer output\n")
    ...
    print("=" * 60)
    print()
    print("[Done] gene_length_normalizer completed successfully!")
