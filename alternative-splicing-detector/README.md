...
可变剪接事件检测+可视化 — 针对浙大生信研究生+陆慧智课题组（DNA损伤修复）方向优化。
...
| output_file | 输出文件路径 | output_alternative_splicing_detector.txt |
...
```bash
# Python工具
cd alternative-splicing-detector
python alternative_splicing_detector.R

# R工具
cd alternative-splicing-detector
Rscript alternative_splicing_detector.R
```
